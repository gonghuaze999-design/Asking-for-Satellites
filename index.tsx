...
const detectGoogleAccess = (): Promise<boolean> => {
  return new Promise(resolve => {
    ...
    // 用一个极小的Google资源测试连通性
    img.src = `https://maps.googleapis.com/maps/api/staticmap?center=0,0&zoom=1&size=1x1&key=${GMAP_KEY}&_t=${Date.now()}`;
  });
};
...
const loadAMap = (): Promise<void> => new Promise((resolve, reject) => {
  ...
  const script = document.createElement('script');
  script.src = `https://webapi.amap.com/maps?v=2.0&key=${AMAP_KEY}&plugin=AMap.MouseTool,AMap.Scale,AMap.ToolBar,AMap.Geocoder`;
