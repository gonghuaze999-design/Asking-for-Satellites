...
import AMapViewComponent, { GoogleMapRef } from '../components/AMapView';
import GoogleMapViewComponent from '../components/GoogleMapView';
const GoogleMapView = (window as any).__MAP_ENGINE__ === 'google' ? GoogleMapViewComponent : AMapViewComponent;
  ...
  const geocodeAdmin = async (names: string[]): Promise<any> => {
    ...
    if (engine === 'google') {
      ...
      return new Promise((resolve, reject) => {
        if (!(window as any).google?.maps) return reject(new Error('Google Maps not loaded'));
        ...
        geocoder.geocode({ address, componentRestrictions: { country: 'CN' } }, (res: any, status: any) => {
          if (status === 'OK' && res[0]) {
            ...
            const geo = { type: 'Feature', geometry: { type: 'Polygon', coordinates: [[[sw.lng(),sw.lat()],[ne.lng(),sw.lat()],[ne.lng(),ne.lat()],[sw.lng(),ne.lat()],[sw.lng(),sw.lat()]]] } };
            onRoiChange(geo); resolve(geo);
      ...
      // 高德 Geocoder
      return new Promise((resolve, reject) => {
        if (!(window as any).AMap) return reject(new Error('AMap not loaded'));
        const geocoder = new AMap.Geocoder({ city: '全国' });
        geocoder.getLocation(address, (status: string, result: any) => {
          if (status !== 'complete' || !result.geocodes?.length) {
            return reject(new Error(`Geocoding failed: ${status}`));
          }
          const g = result.geocodes[0];
          const loc = g.location;
          // 优先用返回的bounds，没有就用location点+偏移量
          let bounds;
          if (g.bounds) {
            const ne_gcj = { lng: g.bounds.northEast?.lng ?? g.bounds.getNorthEast?.()?.lng ?? loc.lng+0.15, lat: g.bounds.northEast?.lat ?? g.bounds.getNorthEast?.()?.lat ?? loc.lat+0.15 };
            const sw_gcj = { lng: g.bounds.southWest?.lng ?? g.bounds.getSouthWest?.()?.lng ?? loc.lng-0.15, lat: g.bounds.southWest?.lat ?? g.bounds.getSouthWest?.()?.lat ?? loc.lat-0.15 };
            const ne = gcj02ToWgs84(ne_gcj.lng, ne_gcj.lat);
            const sw = gcj02ToWgs84(sw_gcj.lng, sw_gcj.lat);
            bounds = { sw, ne };
          } else {
            const wgs = gcj02ToWgs84(loc.lng, loc.lat);
            const delta = 0.12;
            bounds = { sw: { lat: wgs.lat-delta, lng: wgs.lng-delta }, ne: { lat: wgs.lat+delta, lng: wgs.lng+delta } };
          }
          mapRef.current?.fitBounds(bounds);
          const geo = { type: 'Feature', geometry: { type: 'Polygon', coordinates: [[[bounds.sw.lng,bounds.sw.lat],[bounds.ne.lng,bounds.sw.lat],[bounds.ne.lng,bounds.ne.lat],[bounds.sw.lng,bounds.ne.lat],[bounds.sw.lng,bounds.sw.lat]]] } };
          onRoiChange(geo); resolve(geo);
        });
      });
    }
