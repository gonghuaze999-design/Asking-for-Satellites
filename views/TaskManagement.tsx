...
const TaskManagement: React.FC<TaskManagementProps> = ({ tasks, inputData = [], currentRoi, addTask, updateTask, setResults, setActiveTab }) => {
  ...
  const auditAlgorithmWithAI = async (code: string): Promise<{valid: boolean, reason?: string, status: Algorithm['status']}> => {
    ...
    try {
      ...
      return { valid: !!data.valid, reason: data.reason, status: data.valid ? 'VALID' : 'INVALID' };
    } catch (e: any) {
    ...
    try {
      if (outputType === 'LOCAL') {
        ...
        for (let i = 0; i < inputData.length; i++) {
          ...
          try {
            // 直接POST /api/download，服务器代理下载，无VPN也可用
            addUplinkLog(`...Pulling data via server proxy...`);
            const { fileName } = await GeeService.generateSingleLocalUrl(img.id, img.date, selectedAlgoId, currentRoi, experimentName.toUpperCase());
            const response = await fetch('/api/download', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ id: img.id, geometry: currentRoi, algoId: selectedAlgoId, experimentName: experimentName.toUpperCase(), sensingDate: img.date })
            });
