...
const AIProcess: React.FC<AIProcessProps> = ({ 
  ...
  // ── Workflow Algorithm Library (localStorage持久化，与TaskManagement独立) ──
  const WORKFLOW_DEFAULT_ALGOS = [
    { id: 'mode_extract', name: 'Hist Mode Extractor (>0.4)', desc: 'Processes grayscale imagery by filtering pixels > 0.4 and extracting the histogram mode.', author: 'System', createdAt: '2024-01-01', status: 'VALID' as const, isPersistent: true, code: '// Histogram Mode Extractor\nvar result = inputCollection.map(function(image) {\n  return image.select([0]).gt(0.4);\n});\nprint(result.median());' },
    { id: 'veg_binary', name: 'Vegetation Binary Classifier', desc: 'Classifies NDVI input: NDVI > 0.4 sets pixel to 1 (Veg), others to 0.', author: 'System', createdAt: '2024-01-01', status: 'VALID' as const, isPersistent: true, code: '// Vegetation Binary Classifier\nvar result = inputCollection.map(function(image) {\n  return image.select("NDVI").gt(0.4).rename("VegMask");\n});\nprint(result.median());' },
    { id: 'anomaly_det', name: 'Temporal Anomaly Detection', desc: 'Detect deviations from historical baseline modes.', author: 'System', createdAt: '2024-01-01', status: 'VALID' as const, isPersistent: true, code: '// Temporal Anomaly Detection\nvar baseline = inputCollection.mean();\nvar result = inputCollection.map(function(image) {\n  return image.subtract(baseline).rename("Anomaly");\n});\nprint(result.median());' },
  ];
  const [workflowAlgoLib, setWorkflowAlgoLib] = useState(() => {
    try {
      const saved = localStorage.getItem('GEE_WORKFLOW_ALGO_LIB');
      const custom = saved ? JSON.parse(saved) : [];
      return [...WORKFLOW_DEFAULT_ALGOS, ...custom];
    } catch { return WORKFLOW_DEFAULT_ALGOS; }
  });
  const [showWfUploadModal, setShowWfUploadModal] = useState(false);
  const [wfUploadForm, setWfUploadForm] = useState({ name: '', desc: '', code: '', error: '', status: 'IDLE' as 'IDLE' | 'VALID' | 'INVALID' | 'TESTING' });
  const [isWfAuditing, setIsWfAuditing] = useState(false);

  const auditWfAlgorithm = async (code: string) => {
    setIsWfAuditing(true);
    try {
      ...
      return { valid: false, reason: e.message || 'Connection failure.' };
    } finally { setIsWfAuditing(false); }
  };

  const handleWfUploadSubmit = async () => {
    if (!wfUploadForm.name || !wfUploadForm.code) return setWfUploadForm(p => ({ ...p, error: 'Name and code are required.' }));
    setWfUploadForm(p => ({ ...p, error: '' }));
    const result = await auditWfAlgorithm(wfUploadForm.code);
    if (result.valid) {
      const newAlgo = { id: `wf_custom_${Date.now()}`, name: wfUploadForm.name, desc: wfUploadForm.desc, author: 'User', createdAt: new Date().toISOString(), status: 'VALID' as const, isPersistent: true, code: wfUploadForm.code };
      const next = [...workflowAlgoLib, newAlgo];
      setWorkflowAlgoLib(next);
      const toStore = next.filter(a => a.author === 'User');
      localStorage.setItem('GEE_WORKFLOW_ALGO_LIB', JSON.stringify(toStore));
      setShowWfUploadModal(false);
      setWfUploadForm({ name: '', desc: '', code: '', error: '', status: 'IDLE' });
    } else {
      setWfUploadForm(p => ({ ...p, error: result.reason || 'Audit failed.' }));
    }
  };

  const deleteWfAlgorithm = (id: string) => {
    const next = workflowAlgoLib.filter(a => a.id !== id);
    setWorkflowAlgoLib(next);
    const toStore = next.filter((a: any) => a.author === 'User');
    localStorage.setItem('GEE_WORKFLOW_ALGO_LIB', JSON.stringify(toStore));
  };
  ...
  const handleGenerateAIReport = async () => {
    ...
    try {
      ...
      if (data.groundingLinks) setGroundingLinks(data.groundingLinks);
  ...
  return (
    <div className="flex flex-1 overflow-hidden h-full bg-[#050608] font-display">
      <aside className="w-80 border-r border-white/5 bg-[#0a0c10] flex flex-col shrink-0">
        ...
        {/* ── Workflow Algorithm Library ── */}
        <div className="border-t border-white/5 px-6 py-4 space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><Layers size={12} /> Algorithm Library</label>
            <button onClick={() => setShowWfUploadModal(true)} className="text-[8px] font-black text-primary uppercase tracking-widest flex items-center gap-1 hover:opacity-70 transition-opacity">
              <FolderPlus size={11} /> Upload
            </button>
          </div>
          <div className="space-y-1.5 max-h-[200px] overflow-y-auto custom-scrollbar">
            {workflowAlgoLib.map((a: any) => (
              <div key={a.id} className="flex items-center gap-2 p-2.5 bg-black/40 border border-white/5 rounded-xl group hover:border-primary/20 transition-all">
                <BadgeCheck size={12} className={a.author === "System" ? "text-primary shrink-0" : "text-emerald-400 shrink-0"} />
                <div className="flex-1 min-w-0">
                  <p className="text-[9px] font-bold text-slate-300 truncate">{a.name}</p>
                  <p className="text-[7px] text-slate-600 truncate">{a.desc}</p>
                </div>
                {a.author === "User" && (
                  <button onClick={() => deleteWfAlgorithm(a.id)} className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-rose-500 transition-all shrink-0">
                    <Trash2 size={11} />
                  </button>
                )}
              </div>
      ...
      {/* ── Workflow Algo Upload Modal ── */}
      {showWfUploadModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-6">
          <div className="bg-[#0d1117] border border-white/10 rounded-[32px] p-8 w-full max-w-lg space-y-6 shadow-2xl">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="size-10 bg-primary/10 rounded-2xl flex items-center justify-center"><Code2 size={18} className="text-primary" /></div>
                <div>
                  <h3 className="text-[11px] font-black text-white uppercase tracking-widest">Upload Algorithm</h3>
                  <p className="text-[8px] text-slate-500 font-mono">GEE CODE AUDIT REQUIRED</p>
                </div>
              </div>
              <button onClick={() => { setShowWfUploadModal(false); setWfUploadForm({ name: "", desc: "", code: "", error: "", status: "IDLE" }); }} className="text-slate-600 hover:text-white transition-colors"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Algorithm Name *</label>
                <input value={wfUploadForm.name} onChange={e => setWfUploadForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Custom NDRE Calculator" className="w-full bg-black/40 border border-white/10 rounded-xl text-[10px] py-2.5 px-3 text-slate-300 outline-none focus:border-primary/50" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Description</label>
                <input value={wfUploadForm.desc} onChange={e => setWfUploadForm(p => ({ ...p, desc: e.target.value }))} placeholder="Brief description..." className="w-full bg-black/40 border border-white/10 rounded-xl text-[10px] py-2.5 px-3 text-slate-300 outline-none focus:border-primary/50" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">GEE Code *</label>
                <textarea value={wfUploadForm.code} onChange={e => setWfUploadForm(p => ({ ...p, code: e.target.value }))} rows={8} placeholder="// Your GEE JavaScript code here..." className="w-full bg-black/60 border border-white/10 rounded-xl text-[9px] py-3 px-3 text-emerald-400 font-mono outline-none focus:border-primary/50 resize-none" />
              </div>
              {wfUploadForm.error && (
                <div className="flex items-start gap-2 p-3 bg-rose-500/10 border border-rose-500/20 rounded-xl">
                  ...
                  <p className="text-[9px] text-rose-400">{wfUploadForm.error}</p>
                </div>
              )}
            </div>
            <div className="flex gap-3">
              <button onClick={() => { setShowWfUploadModal(false); setWfUploadForm({ name: "", desc: "", code: "", error: "", status: "IDLE" }); }} className="flex-1 py-3 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black text-slate-400 uppercase tracking-widest hover:bg-white/10 transition-all">Cancel</button>
              <button onClick={handleWfUploadSubmit} disabled={isWfAuditing} className="flex-1 py-3 bg-primary text-black rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:scale-[1.02] transition-all disabled:opacity-50">
                {isWfAuditing ? <><Loader2 size={12} className="animate-spin" /> Auditing...</> : <><ShieldCheck size={12} /> Audit & Import</>}
              </button>
            </div>
          </div>
        </div>
