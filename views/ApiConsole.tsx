...
const ApiConsole: React.FC<{ logs: LogEntry[]; addLog: (level: any, msg: string, payload?: any) => void }> = ({ logs, addLog }) => {
  ...
  const handleAiCommand = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 走Nginx /proxy/gemini/ 代理，国内无需VPN
      const geminiKey = process.env.API_KEY || '';
      const geminiPayload = {
        system_instruction: { parts: [{ text: '你现在是卫星遥感平台的 DevOps 与排障专家。协助用户判断平台运行状况。基于日志事实给出建议。回复语言：中文，风格：专业、简洁、极客。' }] },
        contents: [{ parts: [{ text: `SYSTEM_LOG_CONTEXT (Latest 30 events):\n${logContext}\n\nUSER_COMMAND: ${cmd}` }] }]
      };
      const geminiRes = await fetch('/proxy/gemini/v1beta/models/gemini-2.5-flash:generateContent', {
        method: 'POST',
      ...
      const response = { text: geminiData.candidates?.[0]?.content?.parts?.[0]?.text || '' };
