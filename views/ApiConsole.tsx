...
const ApiConsole: React.FC<{ logs: LogEntry[]; addLog: (level: any, msg: string, payload?: any) => void }> = ({ logs, addLog }) => {
  ...
  const handleAiCommand = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 走 /api/console 服务器代理，key在服务器不暴露前端
      const geminiRes = await fetch('/api/console', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          logContext: logs.slice(-30).map(l => `[${l.level}] ${l.message}`).join('\n'),
          command: cmd
        })
      });
      const geminiData = await geminiRes.json();
      if (geminiData.error) throw new Error(geminiData.error);
      addLog('INFO', geminiData.text || 'Audit complete.');
    } catch (err) {
