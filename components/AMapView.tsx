...
const AMapView = forwardRef<GoogleMapRef, AMapViewProps>(({ onGeometryChange, showDrawingTools, center = { lat: 39.9, lng: 116.4 } }, ref) => {
  ...
  const showDrawingRef = useRef(showDrawingTools);

  // 保持ref和prop同步，避免闭包问题
  useEffect(() => { showDrawingRef.current = showDrawingTools; }, [showDrawingTools]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    fitBounds: (bounds: any) => {
      ...
      try {
        const swLng = bounds.sw?.lng ?? 0;
        const swLat = bounds.sw?.lat ?? 0;
        const neLng = bounds.ne?.lng ?? 0;
        const neLat = bounds.ne?.lat ?? 0;
        const sw = wgs84ToGcj02(swLng, swLat);
        const ne = wgs84ToGcj02(neLng, neLat);
        const b = new AMap.Bounds([sw.lng, sw.lat], [ne.lng, ne.lat]);
    ...
    addTileLayer: (urlTemplate: string) => {
      ...
      // 高德v1.4 TileLayer 用字符串模板，占位符为 [x][y][z]
      const amapTemplate = urlTemplate
        .replace('{x}', '[x]').replace('{y}', '[y]').replace('{z}', '[z]');
      const layer = new AMap.TileLayer({
        getTileUrl: amapTemplate,
        zIndex: 100,
        opacity: 1
      });
  ...
  // 监听 showDrawingTools 变化，控制绘制工具开关
  useEffect(() => {
    if (!mouseTool.current) return;
    // v1.4要求：切换模式前先关闭，再重新激活
  ...
  useEffect(() => {
    // 等待AMap SDK加载（index.tsx里已经加载了，这里直接轮询等待）
    const init = () => {
      if (!containerRef.current || mapInstance.current) return;
      if (!(window as any).AMap) {
        setTimeout(init, 100); // 每100ms检查一次SDK是否就绪
        return;
      }
      ...
      // 初始化完成后，如果当前已是DRAW模式则立即激活
      if (showDrawingRef.current) {
        mt.polygon({
          fillColor: '#11b4d4', fillOpacity: 0.15,
          strokeColor: '#11b4d4', strokeWeight: 2
        });
