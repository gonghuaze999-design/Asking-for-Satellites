...
const AMapView = forwardRef<GoogleMapRef, AMapViewProps>(({ onGeometryChange, showDrawingTools, center = { lat: 39.9, lng: 116.4 } }, ref) => {
  ...
  useEffect(() => {
    ...
    loadAMap().then(() => {
      ...
      if (showDrawingTools) {
        ...
        mt.polygon({ fillColor: '#11b4d4', fillOpacity: 0.15, strokeColor: '#11b4d4', strokeWeight: 2 });
        mt.on('draw', (e: any) => {
  ...
  useEffect(() => {
    ...
    if (showDrawingTools) {
      mouseTool.current.polygon({ fillColor: '#11b4d4', fillOpacity: 0.15, strokeColor: '#11b4d4', strokeWeight: 2 });
    } else {
